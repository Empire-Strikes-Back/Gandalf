...
(ns csbooks.mxnet-cnn-text-data
  ...
  ([path embedding-size opts]
   ...
   (with-open [bis (io/input-stream path)
     ...
     (let [word-size                                                (Integer/parseInt (r-string dis))
           ...
           {:keys [max-vectors vocab]
            :or   {max-vectors word-size}} opts
           _                                                        (println "Processing with " {:dim       dim
                                                                                                 :word-size word-size} " loading max vectors " max-vectors)
...
(defn load-mr-data-and-labels
  ...
  (let [positive-examples (mapv #(string/trim %) (-> (slurp (str path "/rt-polarity.pos"))
    ...
    {:sentences x-text
     :labels    (into positive-labels negative-labels)}))
