(ns Gandalf.mxnet-cnn-text-classifier
  (:require [clojure.java.io :as io]
            [clojure.java.shell :refer [sh]]
            [Gandalf.mxnet-cnn-text-data]
...
(defn get-data-symbol [num-embed sentence-size batch-size vocab-size pretrained-embedding]
  (if pretrained-embedding
    ...
    (as-> (sym/variable "data") data
      (sym/embedding "vocab_embed" {:data       data
                                    :input-dim  vocab-size
                                    :output-dim num-embed})
      (sym/reshape {:data         data
                    :target-shape [batch-size 1 sentence-size num-embed]}))))

(defn make-filter-layers [{:keys [input-x num-embed sentence-size]
                           :as   config}
                          filter-size]
  (as-> (sym/convolution {:data       input-x
                          :kernel     [filter-size num-embed]
                          :num-filter num-filter}) data
    (sym/activation {:data     data
                     :act-type "relu"})
...
(defn get-multi-filter-convnet [num-embed sentence-size batch-size vocab-size pretrained-embedding]
  (let [filter-list    [3 4 5]
        ...
        polled-outputs (mapv #(make-filter-layers {:input-x       input-x
                                                   :num-embed     num-embed
                                                   :sentence-size sentence-size} %) filter-list)
        total-filters  (* num-filter (count filter-list))
        concat         (sym/concat "concat" nil polled-outputs {:dim 1})
        hpool          (sym/reshape "hpool" {:data         concat
                                             :target-shape [batch-size total-filters]})
        hdrop          (if (pos? dropout) (sym/dropout "hdrop" {:data hpool
                                                                :p    dropout}) hpool)
        fc             (sym/fully-connected  "fc1" {:data       hdrop
...
(defn train-convnet [{:keys [devs embedding-size batch-size test-size
  ...
  (let [ms-dataset    (Gandalf.mxnet-cnn-text-data/load-ms-with-embeddings mr-dataset-path max-examples embedding-size {:pretrained-embedding pretrained-embedding})
...
(defn -main [& args]
  ...
  ;;; omit max-examples if you want to run all the examples in the movie review dataset
    ...
    #_(train-convnet {:embedding-size 50
                      :batch-size     100
                      :test-size      1000
                      :num-epoch      10})))
